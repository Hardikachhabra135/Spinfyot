...
// ─── Flight Path Line ─────────────────────────────────────────────────────────
// Full-bleed: width:100vw + left:50% + translateX(-50%) guarantees the SVG spans
// from the absolute LEFT viewport edge to the absolute RIGHT viewport edge,
// regardless of any parent container width, padding, or max-width constraints.
const FlightPathLine = ({ pathY, textOpacity }) => (
  ...
  >
    <svg
      ...
      fill="none"
    >
      {/* Path spans full viewBox width: x=0 → left edge, x=1440 → right edge.
          Gentle sine wave matching the reference design. */}
      <motion.path
        d="M 0,380 Q 360,330 720,380 T 1440,380"
        stroke="#99B6F5"
        strokeWidth="2"
        strokeOpacity="0.65"
...
export default function Hero({ onBookCounselling }) {
  ...
  return (
    ...
    >
      ...
      {/* ── AIRPLANE ────────────────────────────────────────────────────────────
          Anchored at { top: 8%, left: 6% } — this IS the final resting position.
          The airplaneFly keyframe moves it FROM upper-right off-screen TO (0,0)
          relative to this anchor, so it naturally lands here.
          Kept outside the inner content div so z-index stacking is independent.
      ── */}
      {!prefersReducedMotion && (
        ...
        >
          <img
            ...
            style={{
              /* clamp ensures it never grows large enough to hit the text below */
              width: 'clamp(80px, 13vw, 200px)',
              objectFit: 'contain',
              filter: 'drop-shadow(0 15px 30px rgba(153,182,245,0.5))',
              /* scaleX(-1): flips the asset so the nose faces RIGHT (toward the girl).
                 rotate(-12deg): gentle nose-up glide angle. Both stay constant. */
