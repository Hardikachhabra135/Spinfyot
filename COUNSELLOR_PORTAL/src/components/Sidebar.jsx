...
export default function Sidebar({ slug }) {
  ...
  const navItems = [
    ...
    { to: `/c/${slug}/chat`, label: "Messages", icon: MessageSquare, badge: unreadCount },
  ];
    ...
    <div className="w-64 bg-slate-900 text-white flex flex-col h-full shadow-xl">
      <div className="p-6 border-b border-slate-800">
        <div className="flex flex-col gap-3">
          ...
          <h2 className="text-xs font-bold tracking-widest text-slate-400 uppercase ml-1">Counsellor</h2>
        </div>
        <p className="text-xs text-slate-400 mt-4 ml-1">Welcome, {counsellor?.name}</p>
      </div>
