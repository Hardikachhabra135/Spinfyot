...
export default function AssignCounsellor() {
  ...
  const handleAssign = async (appointmentId, counsellorId, recordType) => {
    try {
      await api.post('/api/admin/assignments', { appointmentId, counsellorId, recordType }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUnassign = async (appointmentId, recordType) => {
    try {
      await api.post('/api/admin/assignments/unassign', { appointmentId, recordType }, { headers: { Authorization: `Bearer ${token}` } });
      setActiveModalId(null);
      fetchData();
    } catch (error) {
      console.error(error);
                    ...
                    <td className="px-6 py-4 text-right">
                      {activeModalId === `${a._recordType}-${a.id}` ? (
                        <div className="flex flex-wrap justify-end items-center gap-2 max-w-[280px] ml-auto bg-white border border-slate-200 p-2 rounded-xl shadow-lg relative z-10">
                          ...
                          <button 
                            onClick={() => {
                              if (selectedCounsellor) handleAssign(a.id, selectedCounsellor, a._recordType);
                            }}
                          ...
                          {isAssigned && (
                            <button 
                              onClick={() => handleUnassign(a.id, a._recordType)}
                        ...
                        <button 
                          onClick={() => { setActiveModalId(`${a._recordType}-${a.id}`); setSelectedCounsellor(assignment?.counsellorId || ''); }}
